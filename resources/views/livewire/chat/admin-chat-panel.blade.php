<div wire:poll.5s style="display: flex; height: calc(100vh - 12rem); border-radius: 16px; border: 1px solid #e5e7eb; background: white; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1); overflow: hidden;">
    ...
    <div style="width: 340px; flex-shrink: 0; border-right: 1px solid #e5e7eb; display: flex; flex-direction: column;">

        {{-- Header --}}
        <div style="border-bottom: 1px solid #f3f4f6; padding: 12px 16px;">
            <h2 style="font-size: 18px; font-weight: 700; color: #111827; margin: 0;">Admin-Chat</h2>
            <p style="font-size: 12px; color: #6b7280; margin: 4px 0 0;">Tenant-uebergreifend</p>
        </div>
        ...
        <div style="border-bottom: 1px solid #f3f4f6; padding: 12px 16px;">
            ...
            <select wire:model.live="selectedTenantId"
                    style="width: 100%; border-radius: 8px; border: 1px solid #d1d5db; padding: 8px 12px; font-size: 13px; outline: none; margin-bottom: 8px; background: #f9fafb;">
            ...
            <div style="display: flex; gap: 8px;">
                <select wire:model.live="newConversationUserId"
                ...
                <button wire:click="startConversation"
                        style="border-radius: 8px; background: {{ $newConversationUserId ? '#7c3aed' : '#d1d5db' }}; padding: 8px 12px; color: white; border: none; cursor: pointer; transition: background 0.2s;"
        ...
        <div style="flex: 1; overflow-y: auto;">
            @forelse($this->conversations as $conversation)
                ...
                <button
                    ...
                    style="width: 100%; display: flex; align-items: center; gap: 12px; padding: 12px 16px; text-align: left; border: none; cursor: pointer;
                           border-left: 4px solid {{ $isActive ? '#7c3aed' : 'transparent' }};
                ...
                >
                    ...
                    <div style="flex: 1; min-width: 0;">
                        ...
                        {{-- Tenant-Name --}}
                        @if($tenantName)
                            <span style="font-size: 11px; color: #7c3aed; font-weight: 500;">{{ $tenantName }}</span>
                        @endif
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-top: 2px;">
                            <p style="font-size: 12px; color: #6b7280; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; margin: 0;">
                                @if($lastMsg)
                                    @if($lastMsg->type === 'system')
                                        <em>{{ \Illuminate\Support\Str::limit($lastMsg->content, 35) }}</em>
                                    @else
                                        {{ \Illuminate\Support\Str::limit($lastMsg->content, 35) }}
                                    @endif
                                @else
                                    Keine Nachrichten
                                @endif
                            ...
                            @if($unread > 0)
                                <span style="display: inline-flex; height: 20px; min-width: 20px; align-items: center; justify-content: center; border-radius: 10px; background: #7c3aed; padding: 0 6px; font-size: 11px; font-weight: 700; color: white; flex-shrink: 0;">
                                    {{ $unread }}
                                </span>
                            @endif
                        </div>
            ...
            @empty
                <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; color: #9ca3af; padding: 16px;">
                    ...
                    <p style="font-size: 13px; text-align: center; margin: 0;">Noch keine Konversationen.<br>Waehlen Sie einen Mandanten und Benutzer.</p>
                </div>
            @endforelse
        </div>
    </div>
    ...
    <div style="flex: 1; display: flex; flex-direction: column;">
        @if($activeConversationId && $this->otherUser)
            ...
            <div style="border-bottom: 1px solid #f3f4f6; padding: 12px 24px; display: flex; align-items: center; gap: 12px;">
                <div style="width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; border-radius: 50%; background: #ede9fe; color: #6d28d9;">
                    <span style="font-size: 13px; font-weight: 700;">
                ...
                <div>
                    ...
                    <div style="display: flex; align-items: center; gap: 8px;">
                        @if($this->otherUser->tenant)
                            <span style="font-size: 11px; color: #7c3aed; font-weight: 500;">
                                {{ $this->otherUser->tenant->name }}
                            </span>
                        @endif
                        @if($this->otherUser->hasTelegram())
                            <span style="font-size: 11px; color: #6b7280; display: flex; align-items: center; gap: 2px;">
                                📱 Telegram
                            </span>
                        @endif
                    </div>
            ...
            <div style="border-top: 1px solid #f3f4f6; padding: 12px 16px;">
                <form wire:submit="sendMessage" style="display: flex; gap: 8px;">
                    ...
                    <button type="submit"
                            style="border-radius: 12px; background: #7c3aed; padding: 10px 16px; color: white; border: none; cursor: pointer;">
        ...
        @else
            ...
            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; color: #9ca3af;">
                ...
                <p style="font-size: 14px; margin-top: 4px; color: #9ca3af;">oder starten Sie einen neuen Chat mit einem Partner</p>
            </div>
        @endif
    </div>
</div>
