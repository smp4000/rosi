...
<div style="display:flex;flex-direction:column;gap:14px">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
        @forelse($stations as $id => $name)
            <button type="button"
                onclick="rosiSaveEnroll('{{ route('print-agent.enroll', $id) }}', @js($name), this)"
...
<script>
    async function rosiSaveEnroll(url, stationName, btn) {
        const original = btn.innerHTML;
        // 1) Moderne Browser: Ordner waehlen (Geste bleibt durch erstes await erhalten)
        if (window.showSaveFilePicker) {
            try {
                ...
                btn.innerHTML = '✓ Gespeichert: ' + stationName;
